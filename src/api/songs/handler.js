class SongsHandler {
    ...
    async addSongHandler(request, h) {
        try {
            ...
            const {title, year, artist, genre, duration} = request.payload;
            const songId = await this._service.addSong({title, year, artist, genre, duration});
            //set response
    ...
    async editSongByIdHandler(request, h) {
        try {
            ...
            const {id} = request.params;
            const {title, year, artist, genre, duration} = request.payload;
            await this._service.editSongById(id, {title, year, artist, genre, duration});
            return {
                status: 'success',
                data: 'Lagu berhasil diperbaharui'
            }
        } catch (e) {
            const response = h.response({
                status: 'fail',
